...
<Terminal cmd={['$ npx expo install @react-native-vector-icons/material-design-icons @react-native-vector-icons/get-image']} />
...
`getImageSourceSync` is synchronous, so compute the image sources once at module scope rather than on each render. Combine `src` with `sf` to use SF Symbols on iOS and the vector icon on Android — on iOS, `sf` takes precedence over `src`, while Android falls back to `src`.
